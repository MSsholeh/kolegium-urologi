                    ...
                    <div class="kt-widget kt-widget--project-1">
                        ...
                        <div class="kt-widget__body">
                            <div class="kt-widget__stats">
                                <div class="kt-widget__item">
                                    ...
                                    <div class="kt-widget__label">
                                        <span class="btn btn-label-brand btn-sm btn-bold btn-upper">{{ Daster::tanggal($schedule->event_date, 2) }}</span>
                                    </div>
                                </div>
                                <div class="kt-widget__item">
                                    <span class="kt-widget__date">
                                    ...
                                    <div class="kt-widget__label">
                                        <span class="btn btn-label-success btn-sm btn-bold btn-upper">
                                            {{ $schedule->period->name.' ('.Daster::tanggal($schedule->period->started_at).' s/d '.Daster::tanggal($schedule->period->ended_at).')' }}</span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="kt-widget__footer">
                            <div class="kt-widget__wrapper">
                                ...
                                <div class="kt-widget__section">
                                    <small class="mr-2 text-muted">Pendaftaran hanya dapat dilakukan 2 kali dalam satu periode</small>
                                    @if(Auth::check() && ! $registered)
                                    <a href="{{ route('web.schedule.exam.register', $schedule->id) }}" class="btn btn-brand btn-sm btn-upper btn-bold shoot">Daftar Sebagai Peserta</a>
                                    @elseif($registered)
                                        <span class="btn btn-label-{{ config('constant.validation_status.badge.'.$registered->status) }}">Pendaftaran Ke-{{ $registered->submission.' '.$registered->status }}</span>
                                        @if($registered->submission === 1 && $registered->status === 'Ditolak')
                                            <a href="{{ route('web.schedule.exam.register', $schedule->id) }}" class="btn btn-brand btn-sm btn-upper btn-bold shoot ml-3">Daftar Sebagai Peserta</a>
                                        @endif
