...
use App\Models\ParticipantExamRequirement;
    ...
    public function show(ExamSchedule $schedule)
    {
        ...
        $data = [
            ...
            'breadcrumbs' => [
                ...
                $schedule->exam->name.' - '.Daster::tanggal($schedule->event_date) => route('web.schedule.exam.show', $schedule->id)
            ],
            'schedule' => $schedule,
...
//        dd($data['requirements']);
    ...
    public function register(ExamSchedule $schedule)
    {
        $data = [
            'title' => $this->title,
            'breadcrumbs' => [
    ...
    public function store(Request $request, ExamSchedule $schedule)
    {
        ...
        $participant = $schedule->participants()->create([
            'user_id' => $user->id,
            'status' => 'Baru',
            'submission' => $user->countSubmission($schedule)
        ]);
    ...
    public function submitted(ExamSchedule $schedule)
    {
        $data = [
            'title' => $this->title,
            'breadcrumbs' => [
                'Jadwal Ujian & Kursus' => route('web.schedule.index'),
                $schedule->exam->name,
                Daster::tanggal($schedule->event_date) => route('web.schedule.exam.show', $schedule->id),
                'Daftar',
                'Simpan'
            ],
            'schedule' => $schedule,
